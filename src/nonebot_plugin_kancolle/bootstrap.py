...
from .core.equipment_resolver import EquipmentResolver
from .core.resolver import ShipResolver
from .data.enhancer import KcwikiEnhancer
from .data.improvement_enhancer import ImprovementEnhancer
...
_equipment_resolver: EquipmentResolver | None = None
_enhancer: KcwikiEnhancer | None = None
_improvement_enhancer: ImprovementEnhancer | None = None
_renderer: ShipRenderer | None = None
_equipment_renderer: EquipmentRenderer | None = None
...
def get_equipment_resolver() -> EquipmentResolver:
    """获取 EquipmentResolver 单例。"""
    global _equipment_resolver
    if _equipment_resolver is None:
        from .config import get_config
        cfg = get_config()
...
def get_improvement_enhancer() -> ImprovementEnhancer:
    """获取 ImprovementEnhancer 单例（P7.1）。

    用于装备改修数据懒加载：第一次查询装备改修时拉取整个 improve_data.json，
    入库后所有装备改修查询走 SQLite 单点读。
    """
    global _improvement_enhancer
    if _improvement_enhancer is None:
        _improvement_enhancer = ImprovementEnhancer(get_http_client(), get_store())
    return _improvement_enhancer
...
def get_equipment_renderer() -> EquipmentRenderer:
    """获取 EquipmentRenderer 单例（P7）。

    与 ShipRenderer 共享 ImageCache 根目录与 jinja templates 目录。
    装备无立绘拉取，无需 art_cache。
    """
    global _equipment_renderer
    if _equipment_renderer is None:
        from nonebot_plugin_localstore import get_plugin_cache_dir

        from .config import get_config
        cfg = get_config()
        _equipment_renderer = EquipmentRenderer(
            cache_dir=get_plugin_cache_dir() / "render",
            default_theme=cfg.kancolle_render_theme,
            viewport_width=cfg.kancolle_render_viewport_width,
        )
    return _equipment_renderer
...
def _reset_singletons() -> None:
    ...
    global _store, _http_client, _resolver, _equipment_resolver
    global _enhancer, _improvement_enhancer
    global _renderer, _equipment_renderer, _art_cache, _adapters
    if _store is not None:
        _store.close()
    _store = None
    _http_client = None
    _resolver = None
    _equipment_resolver = None
    _enhancer = None
    _improvement_enhancer = None
    _renderer = None
    _equipment_renderer = None
    _art_cache = None
    _adapters = None
